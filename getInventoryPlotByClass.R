...
getInventoryPlotByClass<-function(taxon,assembleages,study){
...
# Select the relevant columns
 result <- merged_data[, c("taxon_clean", "measurement", "age", "id_study", "id_comm", "study_common_taxon", "family", "class")]
...
# Create the plot with symbols
 ...
 # Aggregate the data to calculate median measurement for each age and class
 median_values <- result_filtered %>%
   group_by(age, class) %>%
   summarise(median_measurement = median(measurement))
 
 # Plotting the aggregated data
 plot<-ggplot(median_values, aes(x = age, y = median_measurement, color = class)) +
   geom_line() +
