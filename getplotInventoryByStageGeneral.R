...
getplotInventoryByStageGeneral <- function(mergedAssembleagesTaxon) {
  ...
  plotStage <- ggplot(result_filtered, aes(x = age_group, y = richness, fill = stage)) +
    geom_boxplot(position = position_dodge(width = 0.8), width = 0.7) +  # Ajustar el width seg??n sea necesario
    labs(title = "Richness by Stage from all the studies", x = "Age groups", y = "Richness") +
    theme_minimal() +
