...
getInventoryPlotByClassPresence<-function(result_filtered){
  ...
  class_counts <- aggregate(count ~ class, data = result_filtered, FUN = sum)
  
  # Step 2: Sort the families based on their total count
  sorted_classes <- class_counts[order(class_counts$count, decreasing = TRUE), ]
  
  # Step 3: Select the top 10 families
  top_10_classes <- head(sorted_classes$class, 10)
  # Filter result_filtered based on the top 10 families
  result_filtered <- result_filtered[result_filtered$class %in% top_10_classes, ]
  ...
  plot <- ggplot(result_filtered, aes(x = age, y = total_count, color = class)) +
    geom_line() +
    geom_point(size = 5) +
